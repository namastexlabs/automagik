...
class DiscordAgent(AutomagikAgent):
    ...
    async def run(self, input_text: str, *, multimodal_content=None, system_message=None, message_history_obj: Optional[MessageHistory] = None,
        ...
        # Tracing: Record start time and initialize tracing
        ...
        kwargs_for_sampling = {
            'session_id': channel_payload.get('session_id') if channel_payload else None,
            'user_id': getattr(self.dependencies, 'user_id', None),
            'agent_name': self.name
        }
        sampling_decision = self._should_sample(kwargs_for_sampling)
        ...
        if should_sample and self.tracing and self.tracing.observability:
            trace_ctx = self.tracing.observability.trace_agent_run(
                agent_name=self.name,
                session_id=kwargs_for_sampling.get('session_id', 'unknown'),
        ...
        # Validate token
        if not self.discord_bot_token:
            # Handle tracing for early return
            if trace_ctx:
        ...
        try:
            ...
            # Mark success for tracing
            success = True
            
            # Create response with usage data if available
            ...
            # Add usage data if available (ensure it's serializable)
            if hasattr(result, 'usage') and result.usage:
                # Convert usage to dict to ensure serializability
                try:
                    if hasattr(result.usage, 'model_dump'):
                        response.usage = result.usage.model_dump()
                    elif hasattr(result.usage, '__dict__'):
                        response.usage = result.usage.__dict__
                    else:
                        response.usage = dict(result.usage) if result.usage else None
                except Exception as e:
                    logger.debug(f"Could not serialize usage data: {e}")
                    response.usage = None
            ...
            # Log usage data and track telemetry
            processing_time = (time.time() - start_time) * 1000
            
            # Extract usage from the local scope if available
            usage_data = None
            if 'result' in locals() and hasattr(locals()['result'], 'usage') and locals()['result'].usage:
                try:
                    result_obj = locals()['result']
                    if hasattr(result_obj.usage, 'model_dump'):
                        usage_data = result_obj.usage.model_dump()
                    elif hasattr(result_obj.usage, '__dict__'):
                        usage_data = result_obj.usage.__dict__
                    else:
                        usage_data = dict(result_obj.usage) if result_obj.usage else None
                except Exception as e:
                    logger.debug(f"Could not extract usage data for logging: {e}")
                    usage_data = None
                
                # Log usage to observability if sampled
                if should_sample and self.tracing and self.tracing.observability:
                    ...
                    for provider_name, provider in self.tracing.observability.providers.items():
                        try:
                            logger.info(f"📝 Sending to {provider_name} - usage: {usage_data}")
                            # LangWatch uses log_llm_call method
                            if hasattr(provider, 'log_llm_call'):
                                provider.log_llm_call(
                                    model=getattr(self.dependencies, 'model_name', 'unknown'),
                                    messages=[{"role": "user", "content": input_text[:100] if input_text else ""}],
                                    response=locals().get('result', {}).get('output', '') if 'result' in locals() else '',
                                    usage=usage_data
                                )
                            else:
                                # Fallback for other providers that might have log_usage
                                provider.log_usage(
                                    agent_name=self.name,
                                    model=getattr(self.dependencies, 'model_name', 'unknown'),
                                    usage=usage_data,
                            ...
                            logger.warning(f"Failed to log usage to {provider_name}: {e}")
            
            # Always track in telemetry (anonymous)
            if self.tracing and self.tracing.telemetry:
                try:
                    if success:
                    ...
                    logger.debug(f"Failed to track telemetry: {e}")
