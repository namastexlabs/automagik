...
def create_database_provider() -> DatabaseProvider:
    ...
    # Only auto-detect PostgreSQL if DATABASE_TYPE is not explicitly set
    if "DATABASE_TYPE" not in os.environ:
        # Check for PostgreSQL connection string to auto-detect
        database_url = os.environ.get("DATABASE_URL", "")
        if database_url.startswith("postgresql://") or database_url.startswith("postgres://"):
            logger.info("PostgreSQL DATABASE_URL detected, auto-setting DATABASE_TYPE to postgresql")
            db_type = "postgresql"
