...
async def _sync_message_history_if_needed(
    ...
    if message_history_obj and new_user_id and new_user_id != str(history_user_id):
        ...
        if update_session and session_id:
            try:
                import uuid as _uuid
                from src.db.repository.session import get_session, update_session as _update_session
                from src.db.models import Session
...
async def validate_session_user_consistency(
    ...
    try:
        import uuid as _uuid
        from src.db.repository.session import list_sessions, get_session, update_session
        from src.db.repository.user import list_users, get_user
...
async def cleanup_authentication_orphans(dry_run: bool = True) -> Dict[str, Any]:
    ...
    """
    try:
        from src.db.repository.session import list_sessions
        from src.db.repository.message import list_messages
        from src.db.repository.user import get_user
            ...
            if session.user_id:
                ...
                if not user:
                    ...
                    if not dry_run:
                        ...
                        for message in messages:
                            from src.db.repository.message import delete_message
