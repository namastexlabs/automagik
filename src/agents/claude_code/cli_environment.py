...
class CLIEnvironmentManager:
    ...
    async def auto_commit_with_options(
        ...
        try:
            ...
            commit_msg = message or f"auto-snapshot: {workflow_prefix}workflow run {run_id} progress"
...
🤖 Generated with [Claude Code](https://claude.ai/code)"""
    ...
    async def _merge_to_main(self, workspace: Path, run_id: str) -> Optional[str]:
        ...
        try:
            ...
            # Switch to main branch in the main repository (not worktree)
            main_repo_path = workspace.parent.parent  # Go back to main repo
            ...
            merge_process = await asyncio.create_subprocess_exec(
                "git", "merge", current_branch, "-m", merge_msg,
                cwd=str(main_repo_path),
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
            ...
            if merge_process.returncode == 0:
                ...
                logger.warning(f"Failed to merge to main for run {run_id}: {stderr.decode()}")
                return None
