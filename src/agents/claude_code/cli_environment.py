...
🤖 Generated with [Claude Code](https://claude.ai/code)"""
    ...
    async def cleanup(self, workspace: Path, force: bool = False) -> bool:
        ...
        try:
            ...
            # For persistent worktrees, perform cleanup (auto-cleanup after merge)
            # For non-persistent worktrees, keep for manual review
            if is_persistent:
                logger.info(f"Auto-cleaning persistent worktree: {workspace}")
                # Persistent mode gets auto-cleanup
            else:
                logger.info(f"Keeping non-persistent worktree for manual review: {workspace}")
