#!/usr/bin/env python3
"""Test multimodal functionality for estruturar agent."""
...
def main():
    """Run multimodal test for estruturar agent."""
    print(f"🚀 MULTIMODAL TEST - {AGENT_NAME.upper()}")
    print("="*60)
    
    session_name = f"test-{AGENT_NAME}-multimodal"
    
    # Send image with structure analysis request
    print(f"\n1. Sending image to {AGENT_NAME} agent...")
    result = send_message_with_image(
        "Por favor, me ajude a estruturar e organizar o conteúdo desta imagem",
        session_name
    )
