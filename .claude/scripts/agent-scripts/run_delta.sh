...
if [[ ! -d "$WORK_DIR" ]]; then
    echo -e "${YELLOW}[DELTA]${NC} Workspace not found. Please run setup_worktrees.sh first."
...
if [[ -n "$RESUME_SESSION" ]]; then
    ...
    echo -e "${BLUE}[DELTA]${NC} Resuming session..." | tee -a "$LOG_FILE"
    ...
    echo -e "${BLUE}[DELTA]${NC} Starting Claude with task..." | tee -a "$LOG_FILE"
...
COMPLETION_MSG+="📁 Workspace: am-agents-api\n"
