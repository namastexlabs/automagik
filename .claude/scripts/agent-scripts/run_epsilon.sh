...
# Function to ensure we're running in tmux session
ensure_tmux_session() {
    # Check if we're already in tmux
    if [[ -n "$TMUX" ]]; then
        # We're in tmux - check if we're in the right session
    ...
    if tmux_session_exists && [[ "$FORCE_NEW_SESSION" != "true" ]]; then
        ...
        # SIMPLIFIED VERSION: Re-exec this script inside the existing tmux session with proper env forwarding
        ...
        exec tmux new-session -d -s "$TMUX_SESSION_NAME" -c "$WORK_DIR" \; send-keys "'$0' '$TASK_MSG'" Enter \; attach-session
    fi
}
...
# Run Agent
echo -e "${GREEN}[EPSILON]${NC} Changing to work directory: $WORK_DIR" | tee -a "$LOG_FILE"
cd "$WORK_DIR"

# Verify we're in the right place and MCP tools are available
echo -e "${GREEN}[EPSILON]${NC} Current directory: $(pwd)" | tee -a "$LOG_FILE"
echo -e "${GREEN}[EPSILON]${NC} Checking MCP tools..." | tee -a "$LOG_FILE"
...
if [[ -n "$MCP_CHECK" ]]; then
    ...
    echo -e "${YELLOW}[EPSILON]${NC} Warning: send_whatsapp_message MCP tool not found" | tee -a "$LOG_FILE"
    echo -e "${YELLOW}[EPSILON]${NC} Claude will use script-level notifications only" | tee -a "$LOG_FILE"
fi

if [[ -n "$RESUME_SESSION" ]]; then
    # Resume existing session
    echo -e "${PURPLE}[EPSILON]${NC} Resuming session..." | tee -a "$LOG_FILE"
    
    # Create continuation message
    CONTINUATION_MSG="Continue with the task: ${TASK_MSG:-Continue previous tool development work}"
    ...
    echo -e "${GREEN}[EPSILON]${NC} Executing Claude..." | tee -a "$LOG_FILE"
...
if [[ -z "$CLAUDE_OUTPUT" ]] || [[ ! -s "$OUTPUT_FILE" ]]; then
    echo -e "${RED}[EPSILON]${NC} Error: Claude did not produce any output" | tee -a "$LOG_FILE"
    echo "Check if claude CLI is working: claude -p 'test'" | tee -a "$LOG_FILE"
    send_whatsapp "❌ Epsilon failed to get response from Claude. Check logs at: $LOG_FILE"
    exit 1
fi
