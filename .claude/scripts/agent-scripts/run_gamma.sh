...
echo -e "${GREEN}[GAMMA]${NC} Checking MCP tools..." | tee -a "$LOG_FILE"
...
if [[ -n "$MCP_CHECK" ]]; then
    ...
    echo -e "${YELLOW}[GAMMA]${NC} Warning: send_whatsapp_message MCP tool not found" | tee -a "$LOG_FILE"
    echo -e "${YELLOW}[GAMMA]${NC} Claude will use script-level notifications only" | tee -a "$LOG_FILE"
fi
...
if [[ -n "$RESUME_SESSION" ]]; then
    ...
    echo -e "${GREEN}[GAMMA]${NC} Executing Claude..." | tee -a "$LOG_FILE"
...
if [[ -z "$CLAUDE_OUTPUT" ]] || [[ ! -s "$OUTPUT_FILE" ]]; then
    echo -e "${RED}[GAMMA]${NC} Error: Claude did not produce any output" | tee -a "$LOG_FILE"
    echo "Check if claude CLI is working: claude -p 'test'" | tee -a "$LOG_FILE"
    send_whatsapp "❌ Gamma failed to get response from Claude. Check logs at: $LOG_FILE"
    exit 1
fi
