...
async def ensure_user_uuid_matches_flashed_id(
    ...
    try:
        ...
        from automagik.db.repository.user import create_user, get_user, list_users
        from automagik.db.models import User
        
        # First, try to find existing user by Flashed ID
        users, _ = list_users(page=1, page_size=1000)
        existing_user = None
        ...
        if existing_user:
            # User already exists with correct Flashed ID
            logger.info(f"User already exists with Flashed ID {flashed_user_id}")
            return str(existing_user.id)
        
        # Check if we need to create a new user or update existing one
        phone_user = None
        for user in users:
            if user.user_data:
                user_phone = (
        ...
        if phone_user:
            # Update existing user with Flashed ID
            logger.info(f"Updating existing user {phone_user.id} with Flashed ID {flashed_user_id}")
            
            # Update user data
            updated_user_data = phone_user.user_data or {}
            updated_user_data.update({
                "flashed_user_id": flashed_user_id,
                "flashed_user_name": flashed_user_data.get("name"),
                "flashed_user_phone": phone_number,
                "whatsapp_user_number": phone_number,
                **flashed_user_data
            })
            ...
            updated_user = User(
                id=phone_user.id,
                email=phone_user.email or f"{flashed_user_id}@example.local",
                name=flashed_user_data.get("name") or phone_user.name,
                user_data=updated_user_data
            )
            
            # Update user in database
            create_user(updated_user)  # This is an upsert operation
            
            # If sessions were provided and user conversion happened, update sessions
            if existing_sessions:
                await _update_sessions_after_user_conversion(existing_sessions, str(phone_user.id))
            
            return str(phone_user.id)
        else:
            # Create new user with Flashed UUID
            logger.info(f"Creating new user with Flashed ID {flashed_user_id}")
            
            # Try to use Flashed UUID as user ID
            try:
                user_uuid = _uuid.UUID(flashed_user_id)
            except ValueError:
                # If Flashed ID is not a valid UUID, generate a new one
                user_uuid = _uuid.uuid4()
                logger.warning(f"Flashed ID {flashed_user_id} is not a valid UUID, using generated UUID {user_uuid}")
            ...
            new_user = User(
                id=user_uuid,
                email=f"{flashed_user_id}@example.local",
                name=flashed_user_data.get("name") or f"User {phone_number}",
                user_data=user_data
            )
            ...
            logger.info(f"Created new user {user_uuid} with Flashed ID {flashed_user_id}")
...
async def update_session_user_id(message_history_obj: Any, user_id: str) -> None:
    ...
    try:
        ...
        if message_history_obj and hasattr(message_history_obj, 'session_id'):
            import uuid as _uuid
            from automagik.db.repository.session import get_session, update_session
            from automagik.db.models import Session
            
            session_id = message_history_obj.session_id
            if session_id:
                session_uuid = _uuid.UUID(str(session_id))
                existing_session = get_session(session_uuid)
                ...
                if existing_session:
                    ...
                    if update_session(updated_session):
                        logger.debug(f"Updated session {session_uuid} user_id to: {user_id}")
                    else:
                        logger.warning(f"Failed to update session {session_uuid}")
...
async def make_session_persistent(agent: Any, message_history_obj: Any, user_id: str, force_user_update: bool = False) -> None:
    ...
    """
    try:
        # Update message history user ID
        await update_message_history_user_id(message_history_obj, user_id)
        
        # Update session user ID if needed
        if force_user_update or (message_history_obj and hasattr(message_history_obj, 'user_id') and str(message_history_obj.user_id) != user_id):
            await update_session_user_id(message_history_obj, user_id)
...
def ensure_session_row(session_id: Any, user_id: Optional[Any]) -> None:
    ...
    try:
        import uuid as _uuid
        from automagik.db.repository.session import get_session, create_session
        from automagik.db.models import Session
        
        # Convert to UUID objects
        session_uuid = _uuid.UUID(str(session_id)) if session_id else None
        user_uuid = _uuid.UUID(str(user_id)) if user_id else None
        
        if not session_uuid:
            logger.warning("No session_id provided to ensure_session_row")
            return
        ...
        if not existing_session:
            ...
            new_session = Session(
                ...
                platform="whatsapp",  # Default platform for Flashinho
                agent_id=None,  # Will be set by agent
                created_at=None,  # Will use default
            )
            
            session_id = create_session(new_session)
            logger.info(f"Created new session row: {session_id}")
        else:
            # Update user_id if needed
            if user_uuid and existing_session.user_id != user_uuid:
                from automagik.db.repository.session import update_session
                updated_session = Session(
                    id=session_uuid,
                    user_id=user_uuid,
                    name=existing_session.name,
                ...
                if update_session(updated_session):
                    logger.info(f"Updated session {session_uuid} with user_id {user_uuid}")
                else:
                    logger.warning(f"Failed to update session {session_uuid}")
