#!/usr/bin/env python3
"""Test multimodal functionality for stan_email agent."""
...
def main():
    """Run multimodal test for stan_email agent."""
    print(f"🚀 MULTIMODAL TEST - {AGENT_NAME.upper()}")
    print("="*60)
    
    session_name = f"test-{AGENT_NAME}-multimodal"
    
    # Send image with email context
    print(f"\n1. Sending image to {AGENT_NAME} agent...")
    result = send_message_with_image(
        "I received this image in an email and need help understanding its content for our approval process",
        session_name
    )
