#!/usr/bin/env python3
"""Test multimodal functionality for genie agent."""
...
def main():
    """Run multimodal test for genie agent."""
    print(f"🚀 MULTIMODAL TEST - {AGENT_NAME.upper()}")
    print("="*60)
    
    session_name = f"test-{AGENT_NAME}-multimodal"
    
    # Send image with orchestration request
    print(f"\n1. Sending image to {AGENT_NAME} agent...")
    result = send_message_with_image(
        "I need help analyzing this image and creating a comprehensive workflow to process similar images",
        session_name
    )
    ...
    # Check metadata for any workflow triggers
    metadata = result.get('metadata', {})
    if metadata:
        print(f"Metadata: {metadata}")
        if metadata.get('workflow'):
            print(f"🔧 Workflow triggered: {metadata.get('workflow')}")
