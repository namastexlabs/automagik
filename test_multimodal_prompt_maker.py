#!/usr/bin/env python3
"""Test multimodal functionality for prompt_maker agent."""
...
def main():
    """Run multimodal test for prompt_maker agent."""
    print(f"🚀 MULTIMODAL TEST - {AGENT_NAME.upper()}")
    print("="*60)
    
    session_name = f"test-{AGENT_NAME}-multimodal"
    
    # Send image with prompt creation request
    print(f"\n1. Sending image to {AGENT_NAME} agent...")
    result = send_message_with_image(
        "Please help me create a comprehensive prompt for analyzing images like this one",
        session_name
    )
